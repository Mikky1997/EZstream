...
export async function POST(request: Request) {
  try {
    ...
    const validatedSeason: number | null = season
      ...
      : null;
    const validatedEpisode: number | null = episode
      ? safeParseInt(String(episode), undefined, 1, 1000) ?? null
      : null;
    const validatedProgress = safeParseInt(
      String(progressSeconds || 0),
      0,
      0,
      Number.MAX_SAFE_INTEGER
    );
    const validatedDuration = safeParseInt(
