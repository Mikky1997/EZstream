  ...
  return (
    <main className="min-h-screen bg-gradient-to-b from-gray-900 to-black">
      <div className="container mx-auto px-4 py-8">
        ...
        <p className="text-gray-400 mb-4">Japanese animation series and movies</p>
        ...
        <div className="mb-4 text-sm text-gray-400">
          {isSearching ? (
            <span>Found <span className="text-purple-400">{searchResults.length}</span> anime results</span>
          ) : (
            <>
          ...
          <>
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4">
              {displayItems.map((item) => (
                <MovieCard
                  key={item.id}
                  item={item}
                  mediaType={isSearching ? (item.media_type as 'movie' | 'tv' || 'tv') : (activeTab === 'tv' ? 'tv' : 'movie')}
                />
              ))}
            </div>
            ...
            {displayItems.length === 0 && !loading && (
              <div className="text-center py-20 text-gray-400">
                <p className="mb-4">{isSearching ? 'No anime found for your search.' : 'No anime found in this category.'}</p>
                <p className="text-sm">{isSearching ? 'Try a different search term.' : 'Try selecting a different sort option.'}</p>
              </div>
            )}
