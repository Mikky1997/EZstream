...
export function useFavorites() {
  ...
  const fetchFavorites = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const response = await fetch('/api/user/favorites?limit=50');
      if (response.ok) {
        const data = await response.json();
  ...
  const removeFromFavorites = async (mediaType: 'movie' | 'tv', mediaId: number) => {
    ...
    try {
      const response = await fetch(`/api/user/favorites?mediaType=${mediaType}&mediaId=${mediaId}`, {
        method: 'DELETE',
      });
      
      if (response.ok) {
        setFavorites(prev => prev.filter(item => !(item.media_type === mediaType && item.media_id === mediaId)));
        return true;
      }
    } catch (error) {
  ...
  const isFavorite = (mediaType: 'movie' | 'tv', mediaId: number) => {
    return favorites.some(item => item.media_type === mediaType && item.media_id === mediaId);
  };

  return { favorites, loading, fetchFavorites, addToFavorites, removeFromFavorites, isFavorite };
}
